// Freerunning single-motor bring-up sketch (historical A/B test).
//
// Flash THIS sketch (not ink/ink.ino) when you need to prove the ULN2003 +
// 28BYJ-48 hardware path independent of the V-command protocol:
//
//   scripts/ink.sh bringup
//
// Natural pin order, half-step, ~833 steps/s on motor 0 (D2-D5) only.
// Forward one rev, pause, reverse one rev, repeat. No serial required.
...
void setup() {
    Serial.begin(115200);
    Serial.println("ink bringup motor0 halfstep");
    for (int i = 0; i < 4; i++)
